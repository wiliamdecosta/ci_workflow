...
class Abstract_model extends  CI_Model {
	...
	*/
	...
	public function isUnique($field, $value, $exid = false) {
		...
		$query = "SELECT COUNT(1) AS isunique FROM ".$this->table. " WHERE ".$field." ".$operator.$this->db->escape($value);
		if($exid !== false) {
    ...
    public function generate_seq_id($table_name, $pkey) {
        $seq_name = $table_name."_".$pkey."_seq";
        $sql = "select nextval('".$seq_name."') as seq";
        $query = $this->db->query($sql);
        $row = $query->row_array();
	...
	public function isMultipleUnique() {
        ...
        foreach($this->multiUnique as $field) {
			...
			if(empty($this->record[$field])) continue;
