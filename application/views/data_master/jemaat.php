        ...
        <div class="tab-content no-border">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="input-group">
                        <span class="input-group-btn">
                            <button class="btn btn-danger" type="button" id="btn-downloaddatainduk" onclick="downloadDataInduk()">Download Data Induk <i class="fa fa-file-excel-o"></i></button>
                        </span>
                    </div>
                </div>
            </div>
...
<script>
    $("#tab-2").on("click", function(event) {
        ...
        var grid = $('#grid-table');
        jemaat_id = grid.jqGrid ('getGridParam', 'selrow');
        nama_kk = grid.jqGrid ('getCell', jemaat_id, 'nama_lengkap');
        sektor_id = grid.jqGrid ('getCell', jemaat_id, 'sektor_id');
        alamat_jemaat = grid.jqGrid ('getCell', jemaat_id, 'alamat_jemaat');
        sektor_kode = grid.jqGrid ('getCell', jemaat_id, 'sektor_kode');

        if(jemaat_id == null) {
            swal('Informasi','Silahkan pilih salah satu Kepala Keluarga','info');
            return false;
        }

        loadContentWithParams("data_master.anggota_keluarga", {
            kk_id: jemaat_id,
            nama_kk : nama_kk,
            sektor_id: sektor_id,
            alamat_jemaat: alamat_jemaat,
            sektor_kode:sektor_kode
        });
    });
</script>
...
<script>
    function showData(){
        ...
        jQuery("#grid-table").jqGrid('setGridParam',{
            url: '<?php echo WS_JQGRID."data_master.jemaat_controller/read"; ?>',
            postData: {
                i_search : i_search
            }
        });
    ...
    function downloadDataInduk() {

            var url = "<?php echo WS_JQGRID . "data_master.jemaat_controller/download_datainduk/?"; ?>";
...
<script>
    ...
    jQuery(function($) {
        ...
        jQuery("#grid-table").jqGrid({
            url: '<?php echo WS_JQGRID."data_master.jemaat_controller/crud"; ?>',
            datatype: "json",
            mtype: "POST",
            colModel: [
                {label: 'ID', name: 'jemaat_id', key: true, width: 5, sorttype: 'number', editable: true, hidden: true},
                {label: 'Sektor', name: 'sektor_kode', width: 80},
